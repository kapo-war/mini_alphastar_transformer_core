...
class ArchModel(nn.Module):
    ...
    def forward(self, state, batch_size=None, sequence_length=None, hidden_state=None, return_logits=False, 
        ...
        transformer_output= self.core(embedded_scalar, embedded_entity, embedded_spatial, 
                                              batch_size, sequence_length)
        print('transformer_output.shape:', transformer_output.shape) if debug else None
        print('transformer_output is nan:', torch.isnan(transformer_output).any()) if debug else None

        action_type_logits, action_type, autoregressive_embedding = self.action_type_head(transformer_output, scalar_context, available_actions)
        ...
        if return_logits:
            ...
            if return_baseline:
                winloss_baseline_value = self.winloss_baseline.forward(transformer_output, baseline_state, baseline_opponent_state)
                build_order_baseline_value = self.build_order_baseline.forward(transformer_output, baseline_state, baseline_opponent_state)
                built_units_baseline_value = self.built_units_baseline.forward(transformer_output, baseline_state, baseline_opponent_state)
                upgrades_baseline_value = self.upgrades_baseline.forward(transformer_output, baseline_state, baseline_opponent_state)
                effects_baseline_value = self.effects_baseline.forward(transformer_output, baseline_state, baseline_opponent_state)
                ...
                del transformer_output, winloss_baseline_value, build_order_baseline_value, built_units_baseline_value
    ...
    def mimic_forward(self, state, gt_action, gt_select_units_num, 
        ...
        action_type_logits, action_type, autoregressive_embedding = self.action_type_head(transformer_output, scalar_context, available_actions, gt_action_type)
        if P.skip_autoregressive_embedding:
            autoregressive_embedding = autoregressive_embedding - autoregressive_embedding
            autoregressive_embedding[:] = 0.
        ...
        if multi_gpu_supvised_learning:
            ...
            del transformer_output, baseline_state, baseline_opponent_state
